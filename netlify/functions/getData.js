...
exports.handler = async (event, context) => {
    try {
        ...
        if (!currentUserInfo) { throw new Error("Your email is not authorized."); }
...
const parseNumber = (value) => { if (value === null || value === undefined) return 0; const cleaned = String(value).replace(/[^0-9.-]/g, ''); return parseFloat(cleaned) || 0; };

// --- DATA LOGIC FOR AGENT ---
async function getAgentDashboardData({ sheets, SPREADSHEET_ID, userEmail, currentUserInfo }) {
    const ranges = ['Sales_Log!A2:F', 'Gamification_Tasks!A2:E', 'Agent_Activity_Log!A2:D', 'Sales_Pipeline!A2:F', 'Sales_Agents!A2:F', 'HR_Announcements!A2:E'];
    const response = await sheets.spreadsheets.values.batchGet({ spreadsheetId: SPREADSHEET_ID, ranges });
    const [salesLogData=[], tasksData=[], activityLogData=[], pipelineData=[], agentsData=[], announcementsData=[]] = response.data.valueRanges.map(r => r.values || []);
    
    // Process all data... (This logic is correct and remains the same)
    ...
    // THE FIX IS HERE: Find the most recent announcement
    const latestAnnouncement = announcementsData.sort((a,b) => new Date(b[0]) - new Date(a[0]))[0];
    ...
    return {
        ...
        tasks, history, chartData, leaderboard,
        latestAnnouncement: latestAnnouncement ? { title: latestAnnouncement[2], message: latestAnnouncement[3] } : null
    };
}

// --- DATA LOGIC FOR MANAGER ---
async function getManagerDashboardData({ sheets, SPREADSHEET_ID, currentUserInfo }) {
    const ranges = ['Sales_Agents!A2:F', 'Sales_Log!A2:F', 'Sales_Pipeline!A2:F', 'HR_Announcements!A2:E'];
    const response = await sheets.spreadsheets.values.batchGet({ spreadsheetId: SPREADSHEET_ID, ranges });
    const [agentsData=[], salesLogData=[], pipelineData=[], announcementsData=[]] = response.data.valueRanges.map(r => r.values || []);

    // ... The rest of the manager logic is correct and remains the same ...
    const salesLog = salesLogData.map(r => [new Date(r[0]), r[1], r[2], r[3], parseNumber(r[4]), parseNumber(r[5])]);
    ...
    // THE FIX IS HERE: Find the most recent announcement for the manager too
    const latestAnnouncement = announcementsData.sort((a,b) => new Date(b[0]) - new Date(a[0]))[0];
    ...
    return {
        ...
        pointsLeaderboard,
        chartData: { pipelineFunnel: { labels: Object.keys(pipelineStages), data: Object.values(pipelineStages) }, revenueTrend },
        latestAnnouncement: latestAnnouncement ? { title: latestAnnouncement[2], message: latestAnnouncement[3] } : null
    };
}
